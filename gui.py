class GUI:
    ...
    def run_command(self, calculator):
        '''Запуск основных команд для работы с калькулятором'''
        answer = None
        self.show_menu()
        if self._user_choose > 5:
            print("Такого  варианта операции не существует!\nПовторите операцию.")
            return self.run_command(calculator)
        else:
            if self._user_choose == 5:
                exit(0)
            numbers = list(map(lambda x: int(x), self.get_numbers()))
            if self._user_choose == 4:
                answer = calculator.subtraction(numbers[0], numbers[1])
            if self._user_choose == 3:
                answer = calculator.division(numbers[0], numbers[1])
            if self._user_choose == 2:
                answer = calculator.multiplication(numbers[0], numbers[1])
            if self._user_choose == 1:
                answer = calculator.addition(numbers[0], numbers[1])
            return answer
